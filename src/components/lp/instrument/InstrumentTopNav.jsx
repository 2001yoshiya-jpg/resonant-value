...
export default function InstrumentTopNav() {
    ...
    <header className="fixed top-0 left-0 right-0 z-50 bg-white shadow-sm">
      <div className="flex items-center px-6 h-[90px] gap-8">
        <div className="flex-shrink-0">
          <img
            src="https://media.base44.com/images/public/6a07fe0d9aa87ec73e08277b/6cdcdf294_image.png"
            alt="買取 くらの助"
        ...
        <div className="md:hidden bg-white border-t border-border px-6 pb-4">
          {navLinks.map((l) => (
            <button
              key={l.href}
              onClick={() => scrollTo(l.href)}
              className="block w-full text-left py-3 text-sm font-medium border-b border-border last:border-0"
            >
              {l.label}
            </button>
          ))}
          <div className="flex flex-col gap-3 mt-4">
            <a
              href="tel:0120000000"
              className="flex items-center justify-center gap-2 h-[56px] rounded-full text-white font-bold text-sm"
              style={{ background: "#eb5a9a" }}
            >
              <Phone className="w-4 h-4" />
              電話で無料査定
